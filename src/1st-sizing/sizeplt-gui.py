'''
    ...
    * Far = Federal Aviation Regulation
    Ref. Kenichi Rinoie, "Aircraft Design method -
...
class SizingPlot():
    def __init__(self):
        ...
        self.fit = 25   # fitting parameter for graph
        self.x = []     # x: (W/S)_TO
        ...
        # defensing division by zero (x[0] = 0.0)
        self.x_cr = []
        self.y5 = []
        ...
        # v_cruise: velocity on cruise, where
        #           sound speed on sea level is 340.3 [m/s]
        #           1 [m/s] = 3.281 [ft/s]
        self.v_cruise = self.m_cruise * self. a_ratio * 340.3
        self.v_cruise = 3.281 * self.v_cruise  # [ft/s]

        # -- 1. Take-off parameters --
        # sigma1: density ratio between height and
        #         surface of the sea
        self.sigma1 = 1.0

        # c1 = C_maxTO: maximum coefficient of lift in taking-off
        #               approximately 1.6-2.2
        self.c1 = [1.6, 2.0, 2.2]

        # -- 2. Land parameters --
        # sigma2: density ratio between height and
        #         surface of the sea
        self.sigma2 = self.sigma1
        # c2 = C_maxL: maximum coefficient of lift in landing
        ...
        # -- 3. climb parameters --
        # gamma: minimum of the climb gradient
        #        for the twin engines; Far25.121
        self.gamma = 0.024

        # C3 = C_LmaxTO: maximum coefficient of lift in taking-off
        #                approximately 1.6-2.2
        self.C3 = 2.0

        # calculation parameter lift_drag (= lift-drag ratio)
        # ======================================================================
        # C_L: lift coefficient in clean status
        #      The rule of C_LmaxTO = 1.2^2 * C_L, thus
        self.C_L = self.C3 / (1.2 ** 2)

        # C_FE: equivalent skin friction coefficient
        #       approximately 0.003
        self.C_FE = 0.003

        # SR: area ratio between wetted area and reference area
        #     SR = S_wet/S_ref * decide from the existing airplane
        self.SR = 5.8

        # c_d0: parasite drag; c_d0 = C_FE * S_wet/S_ref
        #       where take-off flap (+0.015)
        #       in legs putted away
        self.c_d0 = self.C_FE * self. SR + 0.015

        #  e_f: Oswalift_drag's efficiency factor (max: e = 1)
        #       where take-off flap in legs putted away
        self.e_f = 0.8

        # ar: aspect ratio * decide from the existing airplane
        self.ar = 8.0

        # C_D: drag coefficient in the clean form
        #      "clean form" is the status that flap and legs are putted away.
        # C_D = c_d0 + C_L^2/eπar
        self.C_D = self.c_d0 + (self.C_L ** 2) / (self.e_f * math.pi * self.ar)
        # ======================================================================
        # Therefore, lift_drag is calculated as follow:
        # lift_drag: Lift-Drag ratio (L/D = C_L/C_D = L by D)
        self.lift_drag = self.C_L / self.C_D

        # L_p: lapse rate
        #      lapse rate is the trust ratio between
        #      height and surface of the sea
        self.lp = 1.0

        # -- 4. cruise parameters --
        # lp_cr: lapse rate on cruise
        #        Height 35000 [ft], M = 0.8; approximately lp = 0.25
        self.lp_cr = 0.25

        # e: Oswalift_drag's efficiency factor (max: e = 1)
        #    in clean form; approximately e = 0.8-0.85
        self.e_clean = 0.8

        # w_r: weight ratio between cruise and take-off
        #     w_r = (W_cr / W_TO) = 0.965
        self.w_r = 0.956
    ...
    def land(self, x, x1, x2, x3, x4,
        ...
        for j in range(self.m):
            ...
            for j in range(4):
                '''
                Unit transformation:
                1 [kg] = 2.2046 [lb], 1 [m] = 3.281 [ft],
                1 [knot] = 1.688 [ft/s]

                density on the sea level is 0.125 [kg*s^2/m^4]
                * "density" is a local variable, so this name
                   is used phase 4. In cruise, it's diffrent
                ...
                # where W_lift = 0.85 W_take-off,
                # so (W/S)_TO = (W/S)_lift / 0.85
                temp2 = temp1 / 0.85
                k.append(temp2)
    ...
    def climb(self, y4, m, lp, lift_drag):
        '''
        Far25.121:
        gamma = lp (T/W)_TO / 2 - 1/(L/D) >= 0.024
        '''
        count = 0
        while True:
            k = 2 * self.lp * (1 / self.lift_drag + self.gamma)
            # consider of the 27.8 celsius temperature,
            # 20% decrease
            k = k / 0.8
            self.y4.append(k)
    ...
    # 4. Cruise
    def cruise(self, x_cr, y5, m, lp_cr,
               e_clean, v_cruise, c_d0, w_r, ar):
        '''
             y = 1/lp_cr * (W_cr/W_TO) * (T/W)_cruise
            (T/W)_cruise = (c_d0 + Δc_d0) * q / (W_cr/W_TO)
                            + (W_cr/W_TO) * x / qeπar
        In this program,
                        k = y
        ...
        for i in range(self.m - self.fit):
            # density [lb*s^2/ft^4]
            density = 0.31 * 0.125 * 2.2046 / (3.281 ** 4)  # 0.002378

            # q: kinematic pressure
            #    deinition q := ρV^2 / 2
            q = 0.5 * density * (self.v_cruise ** 2)

            temp1 = (self.c_d0 - 0.015 + 0.003) * q / (self.w_r * self.x_cr[i])
            temp2 = q * self.e_clean * math.pi * self.ar  # temp2: qeπar
    ...
    # Output
    def output(self, x, y1, y2, y3, m, c,
               x1, x2, x3, x4, y4, x_cr, y5):
        # set labels
        plt.xlabel("(W/S) take-off")
        plt.ylabel("(T/W) take-off")
        plt.title("Sizing-Plot", c="darkred",
        ...
        # define the plot area from 0 to 0.8 for plot 2.
        y = []
        for i in range(self.m):
            i = float(i) * 4 / 1000
            y.append(i)
...
if __name__ == "__main__":
    ...
    sp.take_off(sp.x, sp.y1, sp.y2, sp.y3, sp.m,
                sp.sigma1, sp.c1, sp.s_tofl)
    # 2. Land
    sp.land(sp.x, sp.x1, sp.x2, sp.x3, sp.x4, sp.m,
            sp.sigma2, sp.c2, sp.s_fl)
    # 3. Climb
    sp.climb(sp.y4, sp.m, sp.lp, sp.lift_drag)

    # 4. Cruise
    sp.cruise(sp.x_cr, sp.y5, sp.m, sp.lp_cr, sp.e_clean,
              sp.v_cruise, sp.c_d0, sp.w_r, sp.ar)

    # Output
    sp.output(sp.x, sp.y1, sp.y2, sp.y3, sp.m, sp.c,
              sp.x1, sp.x2, sp.x3, sp.x4, sp.y4,
              sp.x_cr, sp.y5)
